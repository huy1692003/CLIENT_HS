...
const TransactionManager = ({ type }) => {
    ...
    useLayoutEffect(() => {
        const getData = async () => {
            setLoading(true)
            try {
                let res = await paymentService.getPaymentByAdmin(paginate, searchParams, type)
                if (res) {
    ...
    return (
        <>
            <div className="flex justify-between mb-4">
                <h3 className="text-2xl font-bold mb-2">Giao dịch hệ thống</h3>
            ...
            >
                <Form.Item className="mb-0" label={type === 1 ? "BookingID" : 'ID ADS (Mã quảng cáo)'} name={type === 1 ? "BookingID" : 'advertisementID'}>
                    <InputNumber className="w-full" placeholder="Nhập mã " allowClear />
                </Form.Item>
